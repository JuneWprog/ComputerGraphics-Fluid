...
class Scene {
    constructor() {
     ...
     setupScene() {
		...
		var f = this.fluid = new FluidSimulator(density, tankWidth, tankHeight, h, r, maxParticles);
    ...
    simulate() {
        if (!this.paused)
		this.fluid.simulate(
			this.dt, this.gravity, this.flipRatio, this.numPressureIters, this.numParticleIters, 
				this.overRelaxation, this.compensateDrift, this.separateParticles,
				this.obstacleX, this.obstacleY, this.obstacleRadius, this.colorFieldNr);
			this.frameNr++;
    }
	update(){
		this.simulate();
